...
// summary avg 10 (tambahan kecil)
const histKpmEl = document.getElementById("histKpm");
const histAvgEl = document.getElementById("histAvg");
const histFailEl = document.getElementById("histFail");
...
// ===== tambahan kecil untuk summary =====
function kpmFromScore(score){
  if (typeof score !== "string") return NaN;
  const m = score.match(/(\d+)/);
  return m ? Number(m[1]) : NaN;
}
function calcRollingKpm(){
  const vals = runHistory.slice(0,10).map(r => kpmFromScore(r.score)).filter(v => Number.isFinite(v));
  return avg(vals);
}
function updateHistorySummary(){
  if (histKpmEl){
    const v = calcRollingKpm();
    histKpmEl.textContent = Number.isFinite(v) ? String(Math.round(v)) : "-";
  }
  if (histAvgEl) histAvgEl.textContent = fmtDetik(calcRollingAvgTime());
  if (histFailEl) histFailEl.textContent = `${calcRollingFailRate().toFixed(0)} %`;
}
...
function renderHistory(){
  ...
  // cuma tambahan ini, sisanya sama kayak versi awal
