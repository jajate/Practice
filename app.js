(() => {
  ...
  // =========================
  // Config (Vykas-ish)
  // Timer Vykas sering dianggap 4 detik; easy dibuat lebih longgar. :contentReference[oaicite:1]{index=1}
  // =========================
  ...
  // Simple beep pakai WebAudio (tanpa file audio)
  ...
  function beep(freq, ms, gain = 0.04) {
    if (muted || !ac) return;
    try {
      // resume kalau browser belum mengizinkan sebelum interaksi
  ...
  function calcKpm(now) {
    if (!startAtMs) return 0;
    const minutes = Math.max(elapsedMsForKpm(now) / 60000, 1 / 60); // anti div0
  ...
  function winRound(now) {
    ...
    round += 1;
    // round baru langsung reset (timer direset)
    resetRound();
  }

  function failRound(now) {
    // kalau time habis: reset round (tapi skor KPM tetap jalan)
    beep(220, 140, 0.05);
  ...
  function renderBoard() {
    ...
    for (let i = 0; i < target.length; i++) {
      ...
      tiles.push(
        `<div class="${cls.join(" ")}" aria-hidden="true">${escapeHtml(ch)}</div>`
      );
    }
  ...
  function onKeyDown(e) {
    ...
    // biar space / arrow gak scroll
    if ([" ", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) {
      e.preventDefault();
    }

    // Start KPM timer on first valid key input
    if (startAtMs === null) startAtMs = now;

    // Start round timer on first valid key input (mirip “mechanic starts when you begin”)
    ...
    if (isPenaltyActive(now)) {
      // ignore input while frozen
      return;
    }

    // Only accept single char keys
  ...
  function tick() {
    ...
    // time up check (kalau round sudah jalan)
  ...
  el.toggleEasy.addEventListener("change", () => {
    ...
    // biar kerasa efeknya langsung untuk round berikutnya
    resetRound();
  });
